...
| 설정 항목 | 값 | 설명 |
| :--- | :--- | :--- |
| **Build command** | `bun run build` | type-check + Vite 빌드 (dist 생성) |
...
- **커스텀 도메인**: `wrangler deploy`가 `routes`의 `custom_domain`을 바인딩합니다. 자동 바인딩이 실패할 경우 대시보드 **Settings > Domains & Routes > Add custom domain**에서 `cv.shinkeonkim.com`을 직접 추가할 수 있습니다.
- **리다이렉트**: 기존 GitHub Pages 주소(`cv.shinkeonkim.com/`) 유입을 새 주소로 옮기려면 zone에 리다이렉트 규칙(Redirect Rule)을 추가하세요.
